...
class MyMath{
	...
	// return문
	// : 현재 실행중인 메서들르 종료하고 호출된 메서드로 돌아감
	...
	void noReturn() {
		...
		return; // 반환타입이 void이기 떄문에 생략 가능 - 컴파일러가 자동 추가
	}
	
	// 반환값이 있는 메서드
	String yesReturn () {
		String msg = "해당 메서드는 반환 값이 있습니다";
...
//		 else return a;
		...
		// 삼항 연산자
		// if(조건) {실행할 코드 - 조건이 참} else {실행할 코드 - 조건이 거짓}
...
public class Method03 {
	...
	// 3. 메서드 add의 모든 문장이 실행하거나 return문을 만나면
	//    , 호출된 메서드(main)로 돌아와 이후 문장을 실행
	//    , 반환된 값음 value 변수에 할당
	//    , 함수가 void 타입으로 바로 출력하는 경우 console창에 출력
	int value1 = mm.add(10, 20); // 메서드 호출
	int value2 = mm.subtract(20, 20); // 메서드 호출
	int value3 = mm.multiply(5, 3); // 메서드 호출
	double value4 = mm.divide(5, 3); // 메서드 호출
