...
//		나머지 연산자(%): 좌항에 우항을 나는 나머지를 반환
//		8 = 3 *2(몫) + 2 (나머지)
		int reminderResult2 = 8 % 3;
...
//		+= : 좌항에 우항을 더한 값을 다시 좌항에 대입
		num += 3; // num = num + 3; // 13
//		-= : 좌항에 우항을 뺀 값을 다시 좌항에 대입
		num -= 6; // num = num - 6; // 7
//		*= : 좌항에 우항을 곱한 값을 다시 좌항에 대입
		num *= 3; // num = num * 3; // 21
//		/= : 좌항에 우항을 나눈 값을 다시 좌항에 대입
		num /= 5; // num = num / 5; // 4
//      %= : 좌항에 우항을 나눈 나머지 값을 다시 좌항에 대입	
		num %= 3; // num = num % 3 ; // 1
...
//		: 좌항이 우항보다 크나 같으면 true, 아니면 false
		boolean bool3 = 10 > 10; 
		boolean bool4 = 10 >= 10; 
...
//		: 좌항과 우항 중 하나라도 true이면 true, 모두 false인 경우 false
		...
		boolean orBool2 = (orOper1 > orOper2) || false;
		boolean orBool3 = (orOper1 > orOper2) || false;
	
		System.out.println(orBool1); // true
		System.out.println(orBool2); // false		
